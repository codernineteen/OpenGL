...
uniform sampler2D corgiBodyTexture;
uniform sampler2D corgiFaceTexture;
...
{ 
	...
	if (shadeMode == NO_LIGHT)
	{
		if (isTexture == 1) {
			if(isFace == 0) {
				fColor = texture( corgiBodyTexture, texCoord ).rgba;
			}
			else {
				fColor = texture( corgiFaceTexture, texCoord ).rgba;
			}
		} 
		else {
			fColor = color;
	...
	else if (shadeMode == GOURAUD)
	{
		if (isTexture == 1) {
			if(isFace == 0) {
				fColor = color * texture( corgiBodyTexture, texCoord ).rgba;
			}
			else {
				fColor = color * texture( corgiFaceTexture, texCoord ).rgba;
			}
		} 
		else {
			fColor = color;
	...
	else // if (shadeMode == PHONG)
	{
		...
		if (isTexture == 1) {
			if(isFace == 0) {
				fColor = fColor * texture( corgiBodyTexture, texCoord ).rgba;
			}
			else {
				fColor = fColor * texture( corgiFaceTexture, texCoord ).rgba;
			}
		}
	}
} 
